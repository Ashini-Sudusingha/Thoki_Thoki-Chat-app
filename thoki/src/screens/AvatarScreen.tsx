...
export default function AvatarScreen() {
  ...
  return (
    <ImageBackground
      ...
      className=" flex-1 w-full h-full justify-center items-center bg-cover bg-center"
    >
      <SafeAreaView className=" ">
        <StatusBar hidden={true} />

        <View className="bg-white/50 rounded-2xl m-5 pb-[30px]">
          <View className=" items-center ">
            <View>
              <Image
                source={require("../../assets/justlogo.png")}
                className="mx-auto mb-[30px] mt-[30px]"
                style={{ height: 100, width: 100 }}
              />
            </View>
            <View className="items-center">
              <Text className="font-bold text-lg text-slate-700 mb-[5px]">
                Choose a profile image (╹◡◠) here
              </Text>
              <View className="items-center mt-2 h-72">
                <Pressable
                  className="h-[120] w-[120] rounded-full bg-white justify-center items-center border-2 border-gray-400
            border-dashed"
                  onPress={pickImage}
                >
                  {image ? (
        ...
        <View className="mt-2 w-full px-5 mx-auto">
          <Pressable
            disabled={loading ? true : false}
            className="h-14  items-center justify-center  bg-purple-700 rounded-2xl w-[300px]"
            onPress={async () => {
