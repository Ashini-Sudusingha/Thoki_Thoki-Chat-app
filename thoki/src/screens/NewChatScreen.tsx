    ...
    >
      ...
      <View className="flex-1">
        <View className="items-center flex-row mx-4  rounded-2xl px-6 h-14 my-3 bg-gray-100 dark:bg-slate-500">
          ...
          <TextInput
            className="flex-1 text-lg font-bold ps-2 dark:text-white"
            placeholder="Search"
            value={search}
            onChangeText={(text) => setSearch(text)}
          />
        </View>
        <Text className="text-sm ml-5 mt-[10px] font-bold dark:text-white">Add new contact</Text>
        <View className="px-4 my-2 bg-purple-50 mx-4  py-2 rounded-xl">
          <TouchableOpacity
            className="justify-start gap-x-3 flex-row items-center h-14"
            onPress={() => navigation.navigate("NewContactScreen")}
          >
            <View className="bg-purple-800 items-center justify-center w-12 h-12 rounded-full">
              <Feather name="user-plus" size={24} color="white" />
            </View>
            <Text className="text-lg font-bold">New Contact</Text>
          </TouchableOpacity>
        </View>
        <Text className="text-sm ml-5 mt-[10px] font-bold dark:text-white">All contact</Text>
        <View className="mt-2">
          <FlatList
            data={filterdUsers}
